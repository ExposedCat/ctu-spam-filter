...
def evaluate_wordset(weidghtedict, wordset):
    value = 0
    amount_of_words = len(wordset)
    for word in wordset:
        '''
        ...
        '''
        value += (weidghtedict.get(word, (0,0))[0])/amount_of_words 
    return value


def debug_check_corpus(filename, weidghtedict, cutoff):
    ...
    with open(os.path.dirname(filename)+"/"+"!prediction.txt", "w") as file:
        for x in classification:
            ...
            val = evaluate_wordset(weidghtedict, wordset)
            if val > cutoff:
                tag = "OK"
            else:
                tag = "SPAM"
